...
import { IAADPassword } from "./models";
import { IAADDefinition } from "./models";

import axios from "axios";
import { AadOwner } from "./models";
...
export namespace GraphClient {
  ...
  export async function getAadOwners(
    ...
    const response = await instance.get(
      `${baseUrl}/applications/${objectId}/owners`
    );
  ...
  export async function checkPermission(
    ...
    const findUser = owners?.find(
      (owner: AadOwner) => owner.userObjectId === userObjectId
    );
    if (findUser) {
      return true;
    } else {
