  ...
  public async beforeCacheAccess(cacheContext: TokenCacheContext): Promise<void> {
    ...
    if (await fs.pathExists(fileCachePath)) {
      try {
        ...
        throw new Error("read token fail: " + err.message);
      }
    } else {
      try {
        ...
        throw new Error("write token fail: " + err.message);
      }
    }
  }
  ...
  public async afterCacheAccess(cacheContext: TokenCacheContext): Promise<void> {
    if (cacheContext.cacheHasChanged) {
      ...
      try {
        ...
        throw new Error("save token fail: " + err.message);
      }
    }
  }
...
export async function saveAccountId(accountName: string, accountId?: string) {
  ...
  try {
    ...
    throw new Error("save home account id fail: " + err.message);
  }
}
...
export async function loadAccountId(accountName: string) {
  if (await fs.pathExists(accountPath + accountName)) {
    try {
      ...
      throw new Error("read home account id fail: " + err.message);
    }
  }
